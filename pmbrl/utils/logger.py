...
class Logger(object):
    ...
    def log_stats(self, stats):
        reward_stats, info_stats, info_stats_mu = stats
        self.metrics["reward_stats"].append(reward_stats)
        self.metrics["info_stats"].append(info_stats)
        self.metrics["info_stats_mu"].append(info_stats_mu)
        ...
        for key in info_stats_mu:
            info_stats_mu[key] = "{:.2f}".format(info_stats_mu[key])
        self.log("Reward stats:\n {}".format(pprint.pformat(reward_stats)))
        self.log("Information gain stats:\n {}".format(pprint.pformat(info_stats)))
        self.log("Information gain stats mu:\n {}".format(pprint.pformat(info_stats_mu)))
